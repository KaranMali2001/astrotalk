...
function CaretakerProfileContent() {
  ...
  const fetchCaretaker = useCallback(async () => {
    try {
      // Fetch from browse endpoint and find the specific professional
  ...
  const fetchRecentCalls = useCallback(async () => {
    try {
      ...
      // Filter calls for this specific professional
  ...
  useEffect(() => {
    ...
    loadData();
  }, [fetchCaretaker, fetchCurrentUser, fetchRecentCalls]);
  ...
  const handleInitiateCall = useCallback(
    async (callType: "CHAT" | "AUDIO_CALL" | "VIDEO_CALL") => {
      ...
      try {
        ...
        // For chat and audio, show success and navigate back or show call interface
  ...
  if (isLoading) {
    return (
      <div className="min-h-dvh flex flex-col bg-white">
        <DashboardHeader wallet={currentUser?.wallet} />
  ...
  return (
    <div className="min-h-dvh flex flex-col bg-white">
      ...
      <div className="sticky top-0 z-40 bg-white px-4 py-3 flex items-center">
        <Button variant="outline" onClick={() => router.back()} className="text-black hover:bg-gray-100 border border-gray-300 rounded-lg px-4 py-2 flex items-center gap-2">
      ...
      <main className="flex-1 overflow-y-auto pb-20 bg-white">
        ...
        <div className="relative h-40 mx-4 mt-4 rounded-3xl overflow-hidden" style={{ background: "linear-gradient(135deg, #9333ea 0%, #ec4899 20%, #f59e0b 40%, #3b82f6 100%)" }}>
          {/* Decorative books illustration placeholder */}
        ...
        <div className="px-4 mt-4 mb-6">
          ...
          <div className="grid grid-cols-3 gap-4 mb-6">
            ...
            <div className="text-center">
              <div className="text-2xl font-bold text-black">{caretaker.rating?.toFixed(1) || "0.0"}</div>
          ...
          <Card>
            <CardContent className="p-4">
              ...
              <div className="space-y-4">
                {/* Mock reviews - you can replace with actual API call */}
